...
export default function Header() {
  ...
  function changeQtyProduct(id, title, type) {
    for (let i = 0; i < data.length; i++) {
      const el = data[i];
      if (el.id === id && el.title === title) {
        type === "+" ? data[i].qty++ : data[i].qty--;
      }
    }
    console.log(data);
  }
  ...
  for (let i = 0; i < selectedProducts.length; i++) {
    ...
    for (let j = 0; j < data.length; j++) {
      const el2 = data[j];
      if (el2.title === el.title && el2.id === el.id) hasPr = true;
    }
    ...
    if (!hasPr) {
      ...
      const ele = data.find((tt) => tt.title === el.title && tt.id === el.id);
      ele.qty++;
    }
  }
  console.log(selectedProducts);
        ...
        <div className="container-xxl">
          <div className="row d-flex justify-content-between border border-top-0 border-end-0 border-start-0 border-bottom-2">
            ...
            <div className="col-5 d-flex align-items-center justify-content-between">
                  ...
                  <div className="modal-content">
                    ...
                    <div className="modal-body">
                      <form action="#">
                        ...
                        <button className="btn btn-outline-primary me-5 mt-3" onClick={handleSubmit}>
                          Sign in
                        </button>
                        <button
              ...
              <button className="btn bg-none" data-bs-target="#exampleModalToggle" data-bs-toggle="modal">
                <i className="fa-regular fa-user fs-5 pe-2" style={text}></i>Войти в аккаунт
              </button>
            </div>
          </div>
          <nav className="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3">
            ...
            <ul className="nav col-12 col-md-auto mb-2 justify-content-center mb-md-0">
              <li>
                <a href="#" className="nav-link text-dark px-2 link-secondary">
                  Акции
                </a>
              </li>
              <li>
                <a href="#pitsaCard" className="nav-link text-dark px-2">
                  Пицца
                </a>
              </li>
              <li>
                <a href="#sushiCard" className="nav-link text-dark px-2">
                  Суши
                </a>
              </li>
              <li>
                <a href="#water" className="nav-link text-dark px-2">
                  Напитки
                </a>
              </li>
              <li>
                <a href="#addit" className="nav-link text-dark px-2">
                  Закуски
                </a>
              </li>
              <li>
                <a href="#combo" className="nav-link text-dark px-2">
                  Комбо
                </a>
              </li>
              <li>
                <a href="#desert" className="nav-link text-dark px-2">
                  Десерты
                </a>
              </li>
              <li>
                <a href="#sous" className="nav-link text-dark px-2">
                  Соусы
                </a>
              </li>
              <li>
                  ...
                  <div className="modal-content">
                    <div className="modal-header border-0">
                      <h1 className="modal-title fs-5" id="exampleModalLabel">
                        <div className="fs-1">Ваш заказ</div>
                      </h1>
                      <button
                        type="button"
                    ...
                    {/* shu joyga tanlangan productlar chiqadi */}
                    {data.map((pr, i) => {
                      return (
                        <div className="row" key={i}>
                          <div className="col-1">
                            <img src={pr.imgAddress} alt="asas" />
                            <p>{pr.price}</p>
                            <p>{pr.qty}</p>
                            <button onClick={() => changeQtyProduct(pr.id, pr.title, "+")}>+</button>
                            <button onClick={() => changeQtyProduct(pr.id, pr.title, "-")}>-</button>
                          </div>
                    ...
                    {/* <div className="modal-body">{setSelectedProducts}</div> */}
                    <div className="modal-footer d-flex justify-content-between">
                      <span className="fs-4 fw-bold" style={text}>
                        Итого: <span>{totalSumSelectedProducts}</span> ₽
                      </span>
                      <button type="button" style={btn} className="btn text-white btn-lg shopBtn">
                        Оформить заказ
                      </button>
                    </div>
                  </div>
